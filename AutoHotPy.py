...
class Key(object):
    ...
    def up(self):
        ...
        #  leo
        self.auto.interception = InterceptionWrapper()
        self.auto.context = self.auto.interception.interception_create_context()
        hardware_id = ctypes.c_byte * 512
        for i in range(10):
            current_dev = self.auto.interception.INTERCEPTION_KEYBOARD(i)
            if (self.auto.interception.interception_is_keyboard(current_dev)):
                size = self.auto.interception.interception_get_hardware_id(self.auto.context, current_dev, ctypes.byref(hardware_id()), 512);
                if (size != 0):
                    self.auto.default_keyboard_device = current_dev
                    break
...
#            print("#####DEVICE ID:"+str(device))
    ...
    def run(self, macro, trigger_event):
        ...
        """
        if(isinstance(trigger_event,InterceptionKeyStroke)):
